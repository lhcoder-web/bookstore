<template>
  <div class="featured">
    <title-view :label="titleText" btn="" v-if="titleVisible && data && data.length > 0"></title-view>
    <div class="featured-list">
      <div class="featured-item-wrapper">
        <div class="featured-item" v-for="(item, index) in data" :key="index" @click="showBookDetail(item)">
          <div class="img-wrapper">
            <img class="img" :src="item? item.cover : ''">
          </div>
          <div class="content-wrapper">
            <div class="title title-small" ref="title">{{item?item.title:''}}</div>
            <div class="author sub-title-tiny" ref="author">{{item?item.author:''}}</div>
